...
- `--data-value-as-string` (format: `key=123`) same as `--data-value` but interprets value as plain string
...
- `--data-values-env` (format: `DVAL`) can be used to pull out multiple keys from environment variables based on a prefix
  ...
  - assumes values are encoded as YAML
- `--data-values-env-as-strings` (format: `DVAL`) same as `--data-values-env` but interprets values as plain strings
