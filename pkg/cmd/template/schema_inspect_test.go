...
func TestSchemaInspect_errors(t *testing.T) {
	t.Run("when --output is anything other than 'openapi-v3' or 'json-schema", func(t *testing.T) {
		opts := cmdtpl.NewOptions()
		opts.DataValuesFlags.InspectSchema = true
...
foo: doesn't matter
`
		expectedErr := "Data values schema export only supported in OpenAPI v3 and JSON Schema format; specify format with --output=(openapi-v3|json-schema) flag"
	...
	t.Run("when --output is set but not inspecting schema", func(t *testing.T) {
		opts := cmdtpl.NewOptions()
...
func TestSchemaInspect_exports_an_JSONschema_doc(t *testing.T) {
	t.Run("for all inferred types with their inferred defaults", func(t *testing.T) {
		opts := cmdtpl.NewOptions()
		opts.DataValuesFlags.InspectSchema = true
