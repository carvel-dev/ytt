...
./ytt template -f examples/eirini/ --output=tmp/eirini
...
for name in $(ls examples/playground/); do
  if [ "$name" != "example-assert" ] && [ "$name" != "example-load-custom-library" ]; then
    ./ytt template -f examples/playground/${name} > /dev/null
  fi
done

./ytt template -f examples/overlay > /dev/null
./ytt template -f examples/overlay-files > /dev/null
./ytt template -f examples/overlay-regular-files --file-mark file.yml:type=yaml-plain > /dev/null
